...
set.seed(123)  
kmeans_result1 <- kmeans(Rent_factor_data_scaled, centers = 4, nstart = 25) 
print(kmeans_result1)  

Rent_factor_data$cluster <- kmeans_result1$cluster
...
```{r}
silhouette_score <- silhouette(kmeans_result1$cluster, dist(Rent_factor_data_scaled))
...
```{r}
cluster_centers1 <- as.data.frame(kmeans_result1$centers)

cluster_centers1$cluster <- as.factor(1:nrow(cluster_centers1))

cluster_centers_numeric <- cluster_centers1[, -ncol(cluster_centers1)]  # Remove the last column (cluster)
...
cluster_centers_melt <- melt(cluster_centers1, id.vars = "cluster")
...
# clustered by scaled price and encoded counties
```
